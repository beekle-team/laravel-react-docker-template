...
- Tailwind の at-rule は `suspicious.noUnknownAtRules` の `ignore` に列挙する。ルール自体は有効に保つ。v4 のディレクティブ（`@theme` / `@plugin` / `@source`）は `css.parser.tailwindDirectives` で解釈させる
- 既存 scaffold の都合でルールを外す場合は、`linter.rules` でグローバルに `off` にせず `overrides` でファイルを限定する。同じ問題を新しいコードで作ったときに検出できる状態を保つ
